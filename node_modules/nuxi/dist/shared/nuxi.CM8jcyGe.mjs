...
import { c as consola, a as colors, l as logger, p } from './nuxi.S8HN3O_c.mjs';
import 'node:tty';
...
if (!process$1.getBuiltinModule) {
  ...
  process$1.getBuiltinModule = (name2) => {
    if (name2.startsWith("node:") || builtinModules.includes(name2)) {
      return _require.resolve(name2);
    }
  };
}
...
async function runCommand(command, argv = process$1.argv.slice(2), data = {}) {
  argv.push("--no-clear");
